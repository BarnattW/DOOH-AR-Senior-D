...
function Screen({ children, brackets = 'cyan' }) {
  ...
  return (
    <div className="relative min-h-screen bg-black flex flex-col items-center justify-center overflow-hidden ar-grid">
      <div className={`absolute top-8 left-8 w-10 h-10 border-t-2 border-l-2 ${color} opacity-40`} />
      <div className={`absolute top-8 right-8 w-10 h-10 border-t-2 border-r-2 ${color} opacity-40`} />
      <div className={`absolute bottom-8 left-8 w-10 h-10 border-b-2 border-l-2 ${color} opacity-40`} />
      <div className={`absolute bottom-8 right-8 w-10 h-10 border-b-2 border-r-2 ${color} opacity-40`} />
      {children}
    </div>
  );
}
...
function NavButton({ onClick, children, variant = 'primary' }) {
  ...
  return (
    <button onClick={onClick}
      ...
      <span className="relative text-cyan-400 group-hover:text-white transition-colors">{children}</span>
    </button>
  );
}
  ...
  return (
    <Screen>
      ...
      <div className="relative z-10 text-center px-8 max-w-lg">
        <p className="fade-up delay-1 text-xs tracking-[0.4em] text-cyan-500 uppercase mb-5"
          style={{ fontFamily: RAJDHANI }}>
          Augmented Reality · NYC Landmarks
        </p>
        <h1 className="fade-up delay-2 glitch-text text-6xl font-black text-white mb-3"
          style={{ fontFamily: ORBITRON }}>
          AR<span className="text-cyan-400">·</span>DOOH
        </h1>
        <p className="fade-up delay-2 text-xs tracking-[0.5em] text-gray-600 uppercase mb-8"
          style={{ fontFamily: ORBITRON }}>
          Landmark Detection System
        </p>
        <p className="fade-up delay-3 text-gray-400 mb-12 leading-relaxed"
          style={{ fontFamily: RAJDHANI, fontSize: '1.1rem', fontWeight: 300 }}>
          Discover NYC landmarks through your camera.<br />
        ...
        <div className="fade-up delay-4 flex flex-col items-center gap-5">
          ...
          <NavButton onClick={onSkip} variant="ghost">Skip → Go straight to scanner</NavButton>
        </div>
      </div>
    </Screen>
  );
...
function NeedsScreen({ onNext }) {
  const items = [
    { icon: '📷', title: 'A camera', desc: 'Built-in or external — any webcam works' },
    { icon: '🏙️', title: 'A NYC landmark', desc: 'The Edge, One WTC, or Empire State Building' },
    { icon: '☀️', title: 'Good lighting', desc: 'Works best outdoors or in well-lit spaces' },
  ];
  return (
    <Screen>
      <div className="relative z-10 text-center px-8 max-w-lg w-full">
        <ProgressDots total={3} current={0} />
        <p className="fade-up delay-1 text-xs tracking-[0.4em] text-cyan-500 uppercase mb-3"
          style={{ fontFamily: RAJDHANI }}>
          Step 1 of 3
        </p>
        <h2 className="fade-up delay-2 text-3xl font-black text-white mb-10"
          style={{ fontFamily: ORBITRON }}>
          What You'll Need
        </h2>
        <div className="fade-up delay-3 space-y-4 mb-12 text-left">
          {items.map((item, i) => (
            <div key={i} className="flex items-start gap-4 border border-gray-800 px-5 py-4 hover:border-cyan-900 transition-colors">
              <span className="text-2xl mt-0.5">{item.icon}</span>
              <div>
                <p className="text-white text-sm font-semibold mb-0.5" style={{ fontFamily: ORBITRON }}>{item.title}</p>
                <p className="text-gray-500 text-sm" style={{ fontFamily: RAJDHANI }}>{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
...
function HowScreen({ onNext }) {
  const steps = [
    { num: '01', title: 'Point', desc: 'Aim your camera at a NYC landmark — The Edge, One WTC, or Empire State Building.' },
    { num: '02', title: 'Detect', desc: 'Our AI model identifies the building in real-time using computer vision.' },
    { num: '03', title: 'Explore', desc: 'See the building name and confidence score overlaid directly on your screen.' },
  ];
  return (
    <Screen>
      <div className="relative z-10 text-center px-8 max-w-lg w-full">
        ...
        <div className="fade-up delay-3 space-y-5 mb-12 text-left">
          {steps.map((s, i) => (
            <div key={i} className="flex gap-5 items-start">
              <span className="text-cyan-400 font-black text-lg shrink-0 mt-0.5" style={{ fontFamily: ORBITRON }}>
                {s.num}
              </span>
              <div className="border-l border-gray-800 pl-5">
                <p className="text-white text-sm font-semibold mb-1" style={{ fontFamily: ORBITRON }}>{s.title}</p>
                <p className="text-gray-500 text-sm leading-relaxed" style={{ fontFamily: RAJDHANI }}>{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <Screen>
      <div className="relative z-10 text-center px-8 max-w-lg w-full">
        ...
        <h2 className="fade-up delay-2 text-3xl font-black text-white mb-4"
          style={{ fontFamily: ORBITRON }}>
          Camera Access
        </h2>
        <p className="fade-up delay-3 text-gray-400 mb-10 leading-relaxed"
          style={{ fontFamily: RAJDHANI, fontSize: '1.05rem', fontWeight: 300 }}>
          We need your camera to detect landmarks in real-time.<br />
        ...
        {loading ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-xs tracking-[0.3em] text-gray-600 uppercase" style={{ fontFamily: ORBITRON }}>
...
function ScannerScreen({ canvasRef, detected, modelReady, onExit }) {
  return (
    <div className="relative min-h-screen bg-black flex items-center justify-center overflow-hidden">
      <canvas ref={canvasRef} style={{ display: 'block', maxWidth: '100vw', maxHeight: '100vh' }} />
        ...
        <div className="flex items-center gap-2.5">
          {!modelReady ? (
            <>
              ...
              <span className="text-xs tracking-[0.3em] uppercase text-amber-400" style={{ fontFamily: ORBITRON }}>
                Loading model...
              </span>
            </>
          ) : (
            <>
              ...
              <span className="text-xs tracking-[0.3em] uppercase"
                style={{ fontFamily: ORBITRON, color: detected ? '#4ade80' : '#fbbf24' }}>
                {detected ? 'Landmark Detected' : 'Scanning...'}
              </span>
            </>
          )}
        </div>
        <span className="text-xs text-gray-700 tracking-[0.4em]" style={{ fontFamily: ORBITRON }}>AR·DOOH</span>
      </div>
      ...
      <div className="absolute bottom-20 right-8 flex flex-col gap-1.5 pointer-events-none">
        {[
          ...
          <div key={label} className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-600" style={{ fontFamily: RAJDHANI }}>{label}</span>
          </div>
        ))}
      </div>

      {/* Exit */}
      <button onClick={onExit} className="absolute bottom-8 left-1/2 -translate-x-1/2 group">
        <span className="flex items-center gap-2 px-8 py-3 text-xs tracking-[0.3em] uppercase border border-gray-800 hover:border-red-500 text-gray-600 hover:text-red-400 transition-all"
          style={{ fontFamily: ORBITRON }}>
  ...
  return (
    <Screen brackets="red">
      <div className="text-center px-8 max-w-md">
        ...
        <h2 className="text-2xl font-black text-white mb-3" style={{ fontFamily: ORBITRON }}>Camera Blocked</h2>
        <p className="text-gray-500 mb-10 leading-relaxed"
          style={{ fontFamily: RAJDHANI, fontSize: '1.05rem', fontWeight: 300 }}>
          {error || 'Camera access was denied. Please allow camera access in your browser settings and try again.'}
        </p>
        <button onClick={onRetry}
          ...
          <span className="relative text-red-600 group-hover:text-red-400 transition-colors">Try Again</span>
        </button>
      </div>
    </Screen>
  );
...
function App() {
  ...
  return (
    <div>
      ...
      {step === 'welcome'    && <WelcomeScreen    onStart={() => setStep('needs')} onSkip={() => setStep('permission')} />}
      {step === 'needs'      && <NeedsScreen      onNext={() => setStep('how')} />}
      {step === 'how'        && <HowScreen        onNext={() => setStep('permission')} />}
      {step === 'permission' && <PermissionScreen onAllow={requestCamera} loading={permissionLoading} />}
      {step === 'scanner'    && <ScannerScreen    canvasRef={canvasRef} detected={detected} modelReady={!!session} onExit={handleExit} />}
      {step === 'error'      && <ErrorScreen      onRetry={() => setStep('permission')} error={cameraError} />}
    </div>
  );
}
