...
export default function TutorialCard({ title, description, position = 'bottom', onNext, children }) {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => setCanDismiss(true), 1500);
      ...
      >
        <div className="pointer-events-auto mx-auto max-w-sm rounded-2xl border border-white/15 bg-black/80 backdrop-blur-md shadow-2xl px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => canDismiss && setDismissed(true)}
          ...
          {onNext && (
            <button
              type="button"
              onClick={onNext}
              className="mt-3 w-full rounded-xl bg-white/10 hover:bg-white/20 active:scale-95 transition-all duration-150 py-2 text-sm font-semibold text-white/80 hover:text-white"
            >
              Got it →
            </button>
          )}
